...
import "./SingleProductDetailseight.css";
...
function SingleProductDetailseight() {
    const { id } = useParams();
    const customRecomendationItems = useSelector((state) => state.allCart.customRecomendationItems);
    const item = customRecomendationItems.find((p) => p.id === parseInt(id, 10));
...
export default SingleProductDetailseight
